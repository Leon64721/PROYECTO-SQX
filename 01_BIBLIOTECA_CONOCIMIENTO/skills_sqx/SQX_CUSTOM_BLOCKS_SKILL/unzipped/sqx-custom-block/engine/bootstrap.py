...
def _is_value_atom(item: ET.Element) -> bool:
    ...
    if not _params_of(item):       # index stubs like <Item key="RSI"/> have no params
        return False
    # indicator + priceValue + priceRange are the operand families; allow bar/time "other"
...
def build_catalog(config_path: Path, export_path: Path | None,
                  ...
                  snippets_dir: Path | None = None) -> dict:
...
def main(argv: list[str]) -> int:
    ...
    ap.add_argument("--out-dir", default=".", help="where to write catalog.json / catalog.md")
    ...
    if args.auto or args.install:
        if args.install:
            ...
            if not info["is_sqx_install"]:
                print(f"FAIL: no config.xml under install root: {args.install}")
                print(f"  expected: {Path(args.install) / SUBPATHS['config']}")
                return 1
        else:
        ...
        ap.error("provide config.xml, or --install <root>, or --auto")
