...
OUT_ROOT = "E:/PROYECTOS/CLAUDE CODE/STRATEGY QUANT/outputs/NDXm_TICKMILL_20260831"
...
TICK_STEP = "0.01"
POINT_VALUE = "1.0"
DEFAULT_SPREAD = "1.0"      # 100 * tickSize(0.01)
DEFAULT_SLIPPAGE = "0.01"   # 1 * tickSize(0.01)
COMMISSION = "0"
SWAP_LONG = "-4.95"
SWAP_SHORT = "-1.3"
MIN_DISTANCE = "0.0"
...
MC_SPREAD_MIN, MC_SPREAD_MAX = "1.0", "3.0"
...
EDGES = {
 "E1": dict(
    name="Trend Following EMA/ADX", shortname="NDXM_TMILL_E1_H1TF_V4",
 ...
 "E2": dict(
    name="Pullback Continuation EMA/RSI", shortname="NDXM_TMILL_E2_M15PB_V4",
 ...
 "E3": dict(
    name="Volatility Expansion Breakout", shortname="NDXM_TMILL_E3_H1BO_V4",
    timeframe="H1", direction="long",
 ...
 "E4": dict(
    name="Selective Mean Reversion (buy-the-dip)", shortname="NDXM_TMILL_E4_M15MR_V4",
    timeframe="M15", direction="long",
    rules=dict(minConditions=2,maxConditions=4,minExitConditions=0,maxExitConditions=1,
...
if DIAG_MC_MODE:
    _diag_tag = {"off_both": "DIAGOFF", "off_spread": "DIAGNOSPR", "off_slippage": "DIAGNOSLIP"}.get(DIAG_MC_MODE, "DIAG")
    for _e in EDGES.values():
        _e["shortname"] = _e["shortname"].rsplit("_V", 1)[0] + "_" + _diag_tag
    OUT_ROOT = os.path.join(OUT_ROOT, f"_diag_{DIAG_MC_MODE}")
    WORK = os.path.join(OUT_ROOT, "_work")
...
# The "clean master" base ships with TWO <Databanks> blocks per Retest-Task file. Only the one
# with retestSelected="false" is the one SQX actually uses for Input/Output routing (confirmed:
# OOS wrote correctly to the "OOS" databank on disk, matching that block's declared Output, while
# the generic <Databanks> block below it -- always Output=Results/Input=Results, identical in
# every task -- is not what SQX followed). But the retestSelected block itself still carries
# residual OLD databank names from a donor project (exactly the pattern Skill 3 SS12 names as
# blocking: "MONTECARLO RNADOMIZE TRADES Y SKIP TRADES", "MONTECARLO SLIPPAGE Y SPREAD",
# "GESTION MONETARIA" without "- DESACTIVADA"). Real-world symptom observed: MC_TRADES/
# MC_SPREAD_SLIPPAGE never populated (Input pointed at "GESTION MONETARIA", an inactive/empty
# task's databank, and Output wrote to a phantom databank not in config.xml's registry).
...
# Correct the second, non-authoritative <Databanks> block too (for consistency/cleanliness, even
# though it does not appear to be what SQX follows for routing).
...
def patch_setup_charts(txt, timeframe):
    ...
    # timeframe (an M15/H1 strategy retested on D1 bars is not testing the same rules; low
    # trade counts in SPP/WFA results were a direct symptom). Always overwrite, regardless of
    # the tag's current timeframe value.
    def repl(m):
        tag = m.group(0)
...
def edit_mc_spread_slippage_task(root):
    disable_spread = DIAG_MC_MODE in ("off_both", "off_spread")
    disable_slippage = DIAG_MC_MODE in ("off_both", "off_slippage")
    if DIAG_MC_MODE == "off_both":
        # SQX validates that an enabled Cross Check must have >=1 active method underneath it
        # ("MonteCarloRetest method not set" otherwise) -- disabling both child methods while
        # leaving the parent Cross Check itself use="true" is an invalid config, not a
        # runnable "do nothing" test. Disable the parent too for a true null test.
        mcr = root.find(".//MonteCarloRetest")
        if mcr is not None:
            mcr.set("use", "false")
